...
public class SerialActivity extends AppCompatActivity {
    ...
    private Button btOpen, btn_save, btn_unsave;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        btn_unsave = findViewById(R.id.btn_unsave);
        initView();
        ...
        serialHelper = new SerialHelper("dev/ttyS4", 19200) {
            @Override
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (radioGroup.getCheckedRadioButtonId() == R.id.radioButton1) {
                            rec = " Rx:<==" + new String(comBean.bRec, StandardCharsets.UTF_8);
                            ...
                            rec = " Rx:<==" + ByteUtil.ByteArrToHex(comBean.bRec);
                            logListAdapter.addData(comBean.sRecTime   + rec);
                            if (logListAdapter.getData() != null && logListAdapter.getData().size() > 0) {
                                recy.smoothScrollToPosition(logListAdapter.getData().size());
                            }
...
//                botesIndex = position;
        ...
        btOpen.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    Toast.makeText(SerialActivity.this, "IO" + e.getLocalizedMessage() + getString(R.string.tips_cannot_be_opened, e.getMessage()), Toast.LENGTH_SHORT).show();
                    e.printStackTrace();
                } catch (SecurityException se) {
                    Toast.makeText(SerialActivity.this, se.getLocalizedMessage() + "" + getString(R.string.tips_cannot_be_opened, se.getMessage()), Toast.LENGTH_SHORT).show();
                }
            }
        });
                ...
                if (radioGroup.getCheckedRadioButtonId() == R.id.radioButton1) {
                    if (edInput.getText().toString().length() > 0) {
                        if (serialHelper.isOpen()) {
                            ...
                            String rec = edInput.getText().toString();
                            logListAdapter.addData(sDateFormat.format(new Date()) + " Tx:==>" + rec);
                            Log.i("Tx 1", "Tx:==>" + rec);
                            if (logListAdapter.getData() != null && logListAdapter.getData().size() > 0) {
                                recy.smoothScrollToPosition(logListAdapter.getData().size());
                            }
    ...
    private void initView() {
        ...
        btn_unsave.setOnClickListener(view -> back());
...
//        默认选定上次保存的 串口号  校验位  波特率 ；（其他的由于暂时没有供应商特殊指定 暂不保存 ）
    ...
    public static String port_index = "port_index";
    public static String bote_index = "bote_index";
    public static String paruty_index = "paruty_index";
...
//        intent.putExtra("port", spSerial.getSelectedItem().toString());
